...
def test_model(name, folder, script_path, model_type='tflite', benchmark=False):
    ...
    try:
        ...
        if model_type == 'pt':
            model_path = os.path.join(folder, 'models', 'champion_model.pt')
            ...
            model_path = os.path.join(folder, 'models', 'optimized', 'champion_model.tflite')
        ...
        print(f"  [INFO] MCC: {metrics['mcc']:.4f} | Kappa: {metrics['kappa']:.4f}")
